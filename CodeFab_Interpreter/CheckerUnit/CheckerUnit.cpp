...
ValuableValue CheckerUnit::visitUnaryExpr(UnaryExpr& expr) {
	...
	if (r) {
		if (auto result = CheckEvalOperation::evaluateUnaryCalc(expr.getOp(), r->getValue()))
			return *result;
	}
	return nullptr;
}
...
ValuableValue CheckerUnit::visitBinaryExpr(BinaryExpr& expr) {
	...
	if (l_value && r_value) {
		if (auto result = CheckEvalOperation::evaluateBinaryCalc(expr.getOp(), l_value->getValue(), r_value->getValue()))
			return *result;
	}
	return nullptr;
}
