...
> breakpoint를 `{ }`와 같이 동작 수행이 아닌 line에 추가하면 `continue` 시 해당 breakpoint를 무시함.
...
> **최대 재귀 깊이 제한**: 함수 호출 깊이는 **30단계**로 제한됩니다.  
> 초과 시 `RuntimeError: Maximum call stack size exceeded.` 가 발생합니다.  
> 무한 재귀나 지나치게 깊은 재귀는 이 오류로 안전하게 차단됩니다.

```
Func inf() { return inf(); }
inf();    // ❌ RuntimeError: Maximum call stack size exceeded.
```
...
| `RuntimeError` | 실행 중 | 타입 불일치, 미정의 변수, 0 나눗셈, 배열 범위 초과, 인자 수 불일치, **최대 재귀 깊이(30) 초과** 등 |
